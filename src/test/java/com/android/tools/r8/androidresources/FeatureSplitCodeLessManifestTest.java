...
public class FeatureSplitCodeLessManifestTest extends TestBase {
  ...
  public void testFeatureCodeStrippedSetsHasCodeFalse() throws Exception {
    ...
    validateWithBundletool(baseManifest, true);
    ...
    validateWithBundletool(featureWithCodeManifest, true);
    ...
    validateWithBundletool(featureWithoutCodeManifest, false);
  ...
  public void testExplicitHasCodeTrueRewrittenToFalse() throws Exception {
    ...
    validateWithBundletool(featureManifest, false);
  }

  @Test
  public void testCodeLessFeatureFromBeginningDoesNotRewriteHasCode() throws Exception {
    Assume.assumeTrue(parameters.getPartialCompilationTestParameters().isNone());
    TemporaryFolder featureTemp = ToolHelper.getTemporaryFolderForTest();
    featureTemp.create();
    ...
    XmlNode featureManifest =
        getManifestXmlNode(compileResult.getResourceShrinkerOutputForFeature(featureName));
    // Since the feature split had no code from the beginning, R8 is not responsible for updating
    // hasCode. The manifest remains untouched (hasCode defaults to true).
    assertTrue(getEffectiveHasCode(featureManifest));
    validateWithBundletool(featureManifest, true);
  }

  @Test
  public void testSyntheticBaseResourceSplitKeepsHasCode() throws Exception {
    Assume.assumeTrue(parameters.getPartialCompilationTestParameters().isNone());
    TemporaryFolder splitTemp = ToolHelper.getTemporaryFolderForTest();
    splitTemp.create();

    String syntheticBaseName = "synthetic_base_resources";

    // AGP wires the base module's (potentially multi-APK) resources into R8 as resource-only
    // synthetic feature splits: they have an android resource provider and consumer, but no program
    // resource provider (the base's program classes are on the root/base module). Such a split must
    // not be treated as a code-less dynamic feature, i.e., its manifest (the base APK manifest)
    // must
    // keep its code and not be rewritten to android:hasCode="false".
    R8TestCompileResultBase<?> compileResult =
        testForR8(parameters)
            .addProgramClasses(BaseMain.class)
            .addAndroidResources(getBaseResources(temp))
            .addResourceOnlyFeatureSplitAndroidResources(
                getFeatureWithoutCodeResources(splitTemp), syntheticBaseName)
            .enableOptimizedShrinking()
            .addKeepMainRule(BaseMain.class)
            .compile();

    XmlNode syntheticBaseManifest =
        getManifestXmlNode(compileResult.getResourceShrinkerOutputForFeature(syntheticBaseName));
    assertTrue(getEffectiveHasCode(syntheticBaseManifest));
    validateWithBundletool(syntheticBaseManifest, true);
  }

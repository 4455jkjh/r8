...
(function() {
  ...
  const summariesData = JSON.parse(document.getElementById('keepradius-data').textContent);
  ...
  summariesData.forEach(summaryBase64 => {
    try {
      ...
      const message = KeepRadiusSummary.decode(bytes);
      const summary = KeepRadiusSummary.toObject(message, {
        defaults: true,
        enums: String
      });
  ...
  function renderRow(summary) {
    ...
    if (summary.keepRuleKeepRadius && summary.keepRuleKeepRadius.length > 0) {
      const ul = document.createElement('ul');
      ul.style.margin = '0';
      ul.style.paddingLeft = '0';
      ul.style.listStyleType = 'none';
      ul.style.fontSize = '0.9rem';
      summary.keepRuleKeepRadius.forEach(rule => {

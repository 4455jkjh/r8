...
public class ResourceWriter {
  ...
  private static Set<FeatureSplit> getFeatureSplitsWithoutCode(
    ...
    for (DexProgramClass clazz : appView.appInfo().classes()) {
      ...
      if (split != null && !split.isBase() && !splitsWithCode.contains(split)) {
        splitsWithCode.add(split);
      }
    }
    // We should only rewrite if the original feature split had code but we removed it all.
    // We are only responsible for updating the hasCode flag if we were the ones to remove the
    // code.
    return appView
        .options()
        .getFeatureSplitConfiguration()
        .getFeatureSplits(
            split -> !split.isBase() && split.hasCode() && !splitsWithCode.contains(split));
  }
}

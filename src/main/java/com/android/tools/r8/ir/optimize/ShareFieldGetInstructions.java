...
public class ShareFieldGetInstructions extends CodeRewriterPass<AppInfo> {
  ...
  protected CodeRewriterResult rewriteCode(IRCode code) {
    ...
    for (BasicBlock block : ImmutableList.copyOf(code.getBlocks())) {
      ...
      if (successors.size() == 2 && matchingCatchHandlers(successors)) {
      ...
      if (predecessors.size() == 2) {
        ...
        if (firstPredecessor.hasEquivalentCatchHandlers(secondPredecessor)) {
  ...
  private boolean matchingCatchHandlers(List<BasicBlock> blocks) {
    assert blocks.size() == 2;
    return blocks.get(0).hasEquivalentCatchHandlers(blocks.get(1));
  }
